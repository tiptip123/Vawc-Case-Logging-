...
from utils.pdf_export import export_full_pdf, export_filtered_pdf, export_followup_schedule_pdf
from utils.excel_export import export_to_excel, export_followup_schedule_to_excel
...
class ReportsFrame(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        export_grid.grid_columnconfigure((0, 1, 2, 3), weight=1)
        ...
        self.schedule_export_card = self.create_schedule_card(export_grid, 0, 3)
    ...
    def create_schedule_card(self, parent, row, col):
        card = ctk.CTkFrame(parent, fg_color=["white", "#242424"], corner_radius=12, border_width=1, border_color=["#e2e8f0", "#333333"])
        card.grid(row=row, column=col, padx=10, pady=10, sticky="nsew")

        ctk.CTkLabel(card, text="⏰", font=("Arial", 40)).pack(pady=(25, 10))
        ctk.CTkLabel(card, text="Case Follow-up Schedule", font=("Arial", 14, "bold"), text_color=["#1a2a4a", "#f8fafc"]).pack()
        tag = ctk.CTkLabel(card, text="Schedule", font=("Arial", 10, "bold"), text_color="#b45309", fg_color=["#fffbeb", "#78350f"], corner_radius=4, padx=8)
        tag.pack(pady=5)
        ctk.CTkLabel(card, text="Export pending follow-up items with assigned staff and case details.", font=("Arial", 11), text_color=["#64748b", "#94a3b8"], wraplength=200).pack(pady=(10, 10), padx=20)

        button_frame = ctk.CTkFrame(card, fg_color="transparent")
        button_frame.pack(fill="x", padx=20, pady=(0, 10))
        excel_button = ctk.CTkButton(button_frame, text="Excel", fg_color="#16a34a", hover_color="#0f5132", height=38, corner_radius=8, command=self.export_followup_excel)
        pdf_button = ctk.CTkButton(button_frame, text="PDF", fg_color="#2563eb", hover_color="#1e40af", height=38, corner_radius=8, command=self.export_followup_pdf)
        excel_button.pack(side="left", expand=True, fill="x", padx=(0, 5))
        pdf_button.pack(side="left", expand=True, fill="x", padx=(5, 0))

        export_label = ctk.CTkLabel(card, text="Last exported: Never", font=("Arial", 10), text_color=["#94a3b8", "#64748b"])
        export_label.pack(pady=(0, 15))
        card.export_label = export_label
        card.excel_action_button = excel_button
        card.pdf_action_button = pdf_button
        return card
    ...
    def set_export_button_enabled(self, card, enabled=True):
        ...
        if hasattr(card, 'excel_action_button'):
            card.excel_action_button.configure(state="normal" if enabled else "disabled")
        if hasattr(card, 'pdf_action_button'):
            card.pdf_action_button.configure(state="normal" if enabled else "disabled")
    ...
    def export_followup_excel(self):
        self.set_export_status("Exporting follow-up schedule to Excel...")
        self.set_export_button_enabled(self.schedule_export_card, False)
        try:
            self.abuse_combo.configure(values=["Type of Abuse"] + get_abuse_types())
            filter_status = self.status_var.get()
            filter_abuse = self.abuse_var.get()
            ...
            export_followup_schedule_to_excel(
                search_term=self.search_var.get(),
                filter_status=filter_status,
                filter_abuse=filter_abuse,
                filter_year=filter_year
            )
            self.set_export_status("Follow-up schedule exported to Excel successfully.")
            self.update_card_export_label(self.schedule_export_card, f"Last exported: {datetime.now().strftime('%b %d, %Y %I:%M %p')}")
        except Exception as e:
            self.set_export_status("Failed to export follow-up schedule.", error=True)
            messagebox.showerror("Error", str(e))
        finally:
            self.set_export_button_enabled(self.schedule_export_card, True)

    def export_followup_pdf(self):
        self.set_export_status("Exporting follow-up schedule to PDF...")
        self.set_export_button_enabled(self.schedule_export_card, False)
        try:
            self.abuse_combo.configure(values=["Type of Abuse"] + get_abuse_types())
            filter_status = self.status_var.get()
            ...
            export_followup_schedule_pdf(
                search_term=self.search_var.get(),
                filter_status=filter_status,
                filter_abuse=filter_abuse,
                filter_year=filter_year
            )
            self.set_export_status("Follow-up schedule exported to PDF successfully.")
            self.update_card_export_label(self.schedule_export_card, f"Last exported: {datetime.now().strftime('%b %d, %Y %I:%M %p')}")
        except Exception as e:
            self.set_export_status("Failed to export follow-up schedule.", error=True)
            messagebox.showerror("Error", str(e))
