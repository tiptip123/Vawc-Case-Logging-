...
class EditRecordWindow(ctk.CTkToplevel):
    def __init__(self, parent, vawc_no, on_save=None):
        ...
        ctk.CTkLabel(self, text="Edit VAWC Case Record", font=("Arial", 18, "bold"), text_color="#1a2a4a").pack(pady=(15, 10))

        form_frame = ctk.CTkFrame(self, fg_color="#eef0f4")
        form_frame.pack(fill="both", expand=True, padx=15, pady=10)

        ctk.CTkLabel(form_frame, text="Date of Report", anchor="w").pack(fill="x", padx=10, pady=(10, 2))
        ...
        ctk.CTkLabel(form_frame, text="Client Name", anchor="w").pack(fill="x", padx=10, pady=(0, 2))
        self.client_entry = ctk.CTkEntry(form_frame, placeholder_text="Client Name", border_width=2, corner_radius=8)
        self.client_entry.insert(0, self.record[3] or "")
        self.client_entry.pack(padx=10, pady=(0, 10), fill="x")

        ctk.CTkLabel(form_frame, text="Age", anchor="w").pack(fill="x", padx=10, pady=(0, 2))
        ...
        ctk.CTkLabel(form_frame, text="Contact", anchor="w").pack(fill="x", padx=10, pady=(0, 2))
        self.contact_entry = ctk.CTkEntry(form_frame, placeholder_text="Contact", border_width=2, corner_radius=8)
        if self.record[5]:
            self.contact_entry.insert(0, self.record[5])
        self.contact_entry.pack(padx=10, pady=(0, 10), fill="x")

        ctk.CTkLabel(form_frame, text="Birthdate", anchor="w").pack(fill="x", padx=10, pady=(0, 2))
        ...
        ctk.CTkLabel(form_frame, text="Address", anchor="w").pack(fill="x", padx=10, pady=(0, 2))
        ...
        ctk.CTkLabel(form_frame, text="Name of Respondent", anchor="w").pack(fill="x", padx=10, pady=(0, 2))
        self.respondent_entry = ctk.CTkEntry(form_frame, placeholder_text="Name of Respondent", border_width=2, corner_radius=8)
        if self.record[9]:
        ...
        self.attachment_label = ctk.CTkLabel(attach_frame, text="No files selected", anchor="w")
        ...
        ctk.CTkLabel(form_frame, text="Remarks", anchor="w").pack(fill="x", padx=10, pady=(0, 2))
    ...
    def save(self):
        ...
        try:
            ...
            cursor = connection.cursor()
            cursor.execute("""
                UPDATE vawc_logs SET date=?, client_name=?, age=?, contact=?, birthdate=?, address=?, type_of_abuse=?, name_of_respondent=?, case_status=?, attachments=?, remarks=?
                WHERE vawc_no=?
            """, (date, client, int(age) if age else None, contact, birthdate, address, type_of_abuse, respondent, case_status, attachments, remarks, self.vawc_no))
            connection.commit()
            cursor.close()
            connection.close()
